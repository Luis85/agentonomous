...
1. Render the `<select>` immediately with all four options present but
   `disabled`. Default label reads `(detecting…)` or similar — brief
   because the probe is fast (local `import()` resolves in ~1ms for
   already-installed packages, fails instantly for missing ones).
...
5. BDI and learning modes produce working reasoners that tick without
   throwing; behaviour may approximate heuristic.
...
| Risk                                                                                                           | Mitigation                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dynamic `import()` probe misbehaves in Vite's build-time resolution (e.g., bundles the peer even when missing) | Test the probe in both `npm run demo:dev` (esbuild) and `npm run build` (rollup) before declaring the plan complete. If the peer ends up bundled unconditionally, fall back to a static import with try/catch at the adapter construction boundary — the behaviour stays the same from the user's perspective. |
