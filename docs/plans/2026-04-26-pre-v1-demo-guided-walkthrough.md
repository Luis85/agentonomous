...
| 1.3 | Chapters 2-5 wired end-to-end + restart/skip/resume + selector-handle registry | `examples/product-demo/src/demo-domain/walkthrough/chapters/{2..5}.ts`, `examples/product-demo/src/composables/useRegisterSelector.ts`, `examples/product-demo/src/stores/view/selectorHandles.ts`, `examples/product-demo/src/views/TourView.vue` | P1-FR-2, P1-FR-4, P1-FR-5, P1-FR-6, P1-FR-7 | ✅ shipped | [#153](https://github.com/Luis85/agentonomous/pull/153) |
| 1.4 | Playwright `tour-happy-path.spec.ts` | `examples/product-demo/tests/e2e/tour-happy-path.spec.ts`, `examples/product-demo/playwright.config.ts` (webServer wiring), `examples/product-demo/package.json` (`preview:e2e` + `e2e:install` scripts), root `package.json` (`e2e` builds first), `.github/workflows/ci.yml` (new `e2e` job + `ci-gate.needs` entry) | P1-AC-1, P1-AC-5 | ✅ shipped | [#158](https://github.com/Luis85/agentonomous/pull/158) |
...
- 2026-04-27 — slice 1.4 (Playwright `tour-happy-path.spec.ts` + CI gate) shipped via [#158](https://github.com/Luis85/agentonomous/pull/158). **Pillar-1 is now ✅ shipped end-to-end** (planning-doc Tracker table flipped). Notes: (a) Spec drives the production build via Vite preview — `playwright.config.ts` `webServer.command` runs `npm run preview:e2e` (new demo script) against `dist/`; root `npm run e2e` builds lib + demo first. (b) The test asserts URL transitions only on user-action gates (`/tour/<step>`); auto-advance "observe" steps in chapters 2 + 3 chain back-to-back inside a single Vue task and the intermediate URL never settles long enough for `waitForURL` to catch — that's a happy-path artefact, not a bug. (c) Chapter-3 picks `bt` (mistreevous) since it ships as a demo devDependency; the async `setCognitionMode` peer-probe + reasoner construct is absorbed by a 15s `waitForURL` timeout. (d) Chapter-5 captures the export download via `page.waitForEvent('download')` then re-feeds the temp path through `setInputFiles('input[type=file]', ...)`. (e) New `e2e` job in `.github/workflows/ci.yml` rebuilds lib + demo, installs `playwright/chromium` via `npm run e2e:install`, and runs the spec; added to `ci-gate.needs` so the required check fails on a regression. (f) Total user actions in the spec: 6 clicks + 1 select change + 1 file pick = 8 — well under the P1-AC-1 ≤ 25 budget. (g) Local headless run completes in ~500ms (≈ 3s including the demo Vite preview boot via `webServer.reuseExistingServer`).
- 2026-04-26 — slice 1.3 (chapters 2-5 + typed selector registry + `/tour/:step` routing + reset hygiene) shipped via [#153](https://github.com/Luis85/agentonomous/pull/153). Notes: (a) `RegisteredHandle` in `src/stores/view/selectorHandles.ts` is the closed union enforced at every chapter + component call site; the file uses `import type` only so the view layer keeps lint:demo's no-runtime-imports-from-domain rule. (b) `useRegisterSelector(handle)` looks up the host via `data-tour-handle="..."` attributes (querySelector + a `matches` self-check) — same pattern slice 1.2b used for `<HudPanel>`'s ad-hoc registration, generalised so chapters 2-5 don't each re-derive it. (c) New predicate primitives `eventEmittedSinceStep` and `ticksSinceStepAtLeast` read a `stepBaselineTick` field added to `TourCtx`; `useTourProgress` rebases the baseline to 0 when `session.tickIndex` falls (snapshot replay) so chapter-5 import reliably fires on the post-rebuild buffer. (d) `useAgentSession.setCognitionMode(modeId)` probes the peer dep, constructs a fresh reasoner, and calls `agent.setReasoner` before recording `CognitionModeChanged`; `recordUiEvent(type)` is the new push-only entry into `recentEvents` consumed by chapters 2/4/5 (`TracePanelOpened`, `ConfigPreviewOpened`, `SnapshotExported`, `SnapshotImported`). (e) **Chapter-4 ships as a placeholder** — Pillar-4 owns the real preview/commit editor; the slice ships a "Preview JSON" button in `<HudPanel>` that emits the same `ConfigPreviewOpened` event the real editor will emit, so the chapter predicate keeps working when Pillar-4 slice 4.3 lands. (f) `<TourOverlay>` re-evaluates predicates on `recentEvents.length` changes in addition to `session.tickIndex`, so UI events advance the tour without per-component imperative `tour.next()` calls. (g) `TourView.vue` is a thin wrapper around `<PlayView>` that calls `tour.resumeFromRoute(step)` on mount and pushes `tour.currentStepRoutePath` on every `lastStep` change; `resumeFromRoute` is forward-only so a hard reload at `/tour/<earlier-step>` doesn't rewind past the persisted cursor.
