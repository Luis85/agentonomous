...
| 1.4 | Playwright `tour-happy-path.spec.ts` | `examples/product-demo/tests/e2e/tour-happy-path.spec.ts`, `examples/product-demo/playwright.config.ts` (webServer wiring), `examples/product-demo/package.json` (`preview:e2e` + `e2e:install` scripts), root `package.json` (`e2e` builds first), `.github/workflows/ci.yml` (new `e2e` job + `ci-gate.needs` entry) | P1-AC-1, P1-AC-5 | ✅ shipped | TBD |
